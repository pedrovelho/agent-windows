...
!ifndef SERVICELIB
  ...
  !macro FUNC_SERVICE UN
    ...
    lbl_create:
      ...
      !insertmacro CALL_GETPARAM $R1 "machine" "n" "lbl_machine" "${UN}"
        StrCpy $R1 't "$R1"'
      lbl_machine:
 
      !insertmacro CALL_GETPARAM $R2 "user" "n" "lbl_user" "${UN}"
        StrCpy $R2 't "$R2"'
      lbl_user:
 
      !insertmacro CALL_GETPARAM $R3 "password" "n" "lbl_password" "${UN}"
        StrCpy $R3 't "$R3"'
      lbl_password:
 
      !insertmacro CALL_GETPARAM $R4 "interact" "0x10" "lbl_interact" "${UN}"
        StrCpy $6 0x10
        IntCmp $R4 0 +2
        IntOp $6 $6 | 0x100
        StrCpy $R4 $6
      ...
      !insertmacro CALL_GETPARAM $R5 "autostart" "0x3" "lbl_autostart" "${UN}"
        StrCpy $6 0x3
        IntCmp $R5 0 +2
        StrCpy $6 0x2
        StrCpy $R5 $6
      ...
      !insertmacro CALL_GETPARAM $R6 "path" "n" "lbl_path" "${UN}"
      lbl_path:
 
      !insertmacro CALL_GETPARAM $R7 "display" "$2" "lbl_display" "${UN}"
      lbl_display:
 
      !insertmacro CALL_GETPARAM $R8 "description" "$2" "lbl_description" "${UN}"
      lbl_description:
    ...
    ; is installed
    lbl_installed:
      !insertmacro CALL_GETPARAM $7 "action" "" "lbl_good" "${UN}"
        StrCpy $3 $7
        Goto lbl_select
